    ...
    public class BasicSettingsController : BaseController
    {
        ...
        public ActionResult Index()
        {
            ...
            using (var context = new SBDbContext(Session["DBName"].ToString()))
            {
                ...
                List<BasicSettingsView> basicSettings = new List<BasicSettingsView>();
                ...
                basicSettings = (from os in context.AppParams
                                 where os.appIsActive == true && os.AccountProfileId == apId
                                 select new BasicSettingsView
                                 {
                                     appUID = os.appUID,
                ...
                {
                    case 2:
                        foreach (var os in basicSettings)
                        {
                ...
                model.BasicSettings = basicSettings.ToList();
                var defaultnotesetting = basicSettings.FirstOrDefault(x => x.appParam == "DefaultSalesNotes" && x.AccountProfileId == apId);
        ...
        public ActionResult Edit(FormCollection formCollection)
        {
            using (var context = new SBDbContext(Session["DBName"].ToString()))
            {
                ...
                context.SaveChanges();
                TempData["message"] = string.Format(Resources.Resource.SavedOkFormat, Resources.Resource.BasicSettings);
                return RedirectToAction("Index", "BasicSettings");
            }
        }
