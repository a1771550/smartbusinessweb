...
function addRefRow(
    ...
    //       );
    const salesamt: string = formatnumber(
        getSalesLnAmt(
            refundsalesln?.qtyToRefund!,
            refundsalesln?.rtlSellingPrice!,
    ...
    html += `<td class="text-right"><input type="number" name="amount" class="amount text-right" ${areadonly} value="${salesamt}" /></td>`;
...
function salesDetail(itemcode, seq) {
    ...
    if (selectedItemCode === "") {
        ...
        if (RefundableSalesList.length > 0) {
            ...
            if (seq > 0) {
                ...
                //   $refundableQty = fillInRefundRow(
                //     e,
                //     $refundableQty,
                //     refundsaleslns.length === 1
                //   );
                // });
                ...
                //   refundsalesln,
                //   $refundableQty,
                //   RefundableSalesList.length === 1
                // );
                // $refundableQty.val(refundsalesln.refundableQty);
                // $target.find("td:last").find(".seq").val(refundsalesln.rtlSeq);
                // $("#btnRefund").prop("disabled", false).css({ cursor: "pointer" });
                // // return false;
                // updateRefRow();
            } else {
...
function fillInRefundRow(
    ...
    if (refundsalslnlengthEq1) {
        ...
        $target.find("td:eq(2)").find(".refundedqty").val("");
    }
