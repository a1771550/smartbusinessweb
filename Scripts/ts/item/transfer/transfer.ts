...
let ivqtylist: IIvQty[] = $infoblk.data("ivqtylist");
let ivdelqtylist: IIvDelQty[] = $infoblk.data("ivdelqtylist");

let hasIvOnly: boolean = $infoblk.data("hasivonly") === "True";
...
$(document).on("click", "#btnSave", function () {
   ...
   // return;
    if (TransferList.length > 0) {
        $.ajax({
            type: "POST",
            url: "/Transfer/ProcessTransferIOVari",
            data: {
...
$(function () {
    ...
    //console.log("ivqtylist:", ivqtylist);
    //console.log("ivdelqtylist:", ivdelqtylist);
    if (hasIvOnly) {
        $(".tblTransfer tbody tr").eq(1).find(".ivqtytf").trigger("focus");
    } else {
